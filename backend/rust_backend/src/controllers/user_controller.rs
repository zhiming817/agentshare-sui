...
pub async fn get_user_by_id(
    ...
    match UserService::get_user_by_id(db.get_ref(), *id).await {
        Ok(Some(user)) => {
            let response = ApiResponse {
                success: true,
                data: Some(UserResponse {
...
/// PUT /api/users/:id/nickname - 更新昵称
pub async fn update_nickname(
    db: web::Data<DatabaseConnection>,
    id: web::Path<i64>,
    request: web::Json<UpdateNicknameRequest>,
) -> Result<HttpResponse> {
    match UserService::update_nickname(db.get_ref(), *id, request.nickname.clone()).await {
        Ok(user) => {
            let response = ApiResponse {
                success: true,
                data: Some(UserResponse {
