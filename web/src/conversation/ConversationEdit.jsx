...
export default function ConversationEdit() {
  ...
  const loadResumeDetail = async () => {
    ...
    try {
      ...
      const resume = await conversationService.getResumeDetail(id, owner);
      
      console.log('加载的会话数据:', resume);
    ...
    try {
      ...
      try {
        ...
        if (err.message.includes('NoAccess') || err.message.includes('无权访问')) {
          throw new Error('您不在会话的访问白名单中，无法编辑');
        } else {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // 如果有加密密钥和 CID，说明原来是加密会话，需要重新加密
      let newBlobId = null;
      let newSalt = null;
      if (encryptionKey && currentCid) {
        console.log('🔐 检测到加密会话，开始重新加密...');
        
        // 转换表单数据为 API 格式（用于加密）
        const dataToEncrypt = transformConversationData(formData, walletAddress);
      ...
      console.log('更新会话数据:', updateData);
      
      // 调用更新会话 API
      await conversationService.updateResume(id, updateData);
